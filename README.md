...
## 스케줄러 설정 오류와 다중 실행 방어 

고정 지출 자동화 로직을 구현하면서, 빠른 기능 검증을 위해 스케줄러를 매분 실행되도록 설정했습니다.
하지만 이 설정이 실제 운영 환경에 그대로 배포되는 실수가 있었습니다.

원래대로라면 하루에 한 번만 실행되어야 할 로직이 하루에 1440번이나 실행되면서 불필요한 서버 자원이 소모되었습니다.

다만, 이런 상황에서도 데이터가 중복으로 쌓이는 치명적인 오류는 발생하지 않았습니다. 데이터베이스 쿼리를 작성할 때 MERGE INTO 문을 활용하여
동일한 데이터가 중복 저장되지 않도록 미리 대비해 두었기 때문입니다. 
...
이를 통해 로직이 중복 실행되는 예외 상황에서도 데이터의 무결성을 지켜낼 수 있었습니다.

## 프로젝트를 통해 배운 점

이번 프로젝트를 진행하며 단순히 눈에 보이는 기능을 완성하는 것을 넘어, 실제 서비스의 운영과 안정성을 미리 고민하는 백엔드 설계의 중요성을 깊이 체감했습니다. 일반 로그인과 소셜 로그인처럼 인증 방식이 다르더라도 계정 잠금, 휴면, 탈퇴와 같은 사용자 상태는 하나의 통일된 기준으로 제어해야 함을 깨달으며 종합적인 아키텍처 설계의 필요성을 배웠습니다. 또한 스케줄러가 과다 실행되는 상황을 겪으면서 애플리케이션 코드뿐만 아니라 데이터베이스 단에서도 MERGE INTO와 같은 예방책을 마련해 두는 것이 서비스 장애를 막는 든든한 안전장치가 된다는 것을 확인했습니다. 이를 통해 테스트를 마친 코드를 실제 서버에 배포할 때는 서버 자원과 직결되는 설정들이 운영 환경에 맞게 잘 변경되었는지 반드시 점검해야 한다는 교훈을 얻었습니다.
