...
## 스케줄러 실행 주기 오류와 DB 단의 데이터 무결성 방어

고정 지출 자동화 로직을 구현하면서, 빠른 기능 검증을 위해 스케줄러를 매분 실행되도록 설정했습니다.
하지만 이 테스트용 설정이 실제 운영 환경에 그대로 배포되는 실수가 발생했습니다.

- 결과 및 방어 : 해당 로직은 하루 한번만 실행되어도 충분하지만, 설정 누락으로 인해 매분 실행되며 불필요한 서버 자원을 소모했습니다.
하지만 다행히도 고정 지출 로직 내부에 작성해 둔 MERGE INTO 쿼리 덕분에 하루에 1440번 스케줄러가 돌아갔음에도 불구하고 데이터 중복 삽입이라는
치명적인 버그는 방어할 수 있었습니다.
...
이를 통해 인증 로직 설계에서는 단순한 로그인 기능 구현을 넘어서
사용자 상태와 보안 요소를 함께 고려해야 한다는 점을 배울 수 있었습니다.

또한, 스케줄러 자동화와 예외 상황을 겪으며 애플리케이션 로직뿐만 아니라 DB 쿼리 레벨에서의 데이터 무결성 방어가 시스템 안정성에 얼마나 큰 기여를 하는지 깨달았습니다. 
