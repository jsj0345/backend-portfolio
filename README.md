...
하지만 이 테스트용 설정이 실제 운영 환경에 그대로 배포되는 실수가 있었습니다.
해당 로직은 하루 한번만 실행되어도 충분하지만, 설정 누락으로 인해 매분 실행되며 불필요한 서버 자원을 소모했습니다.
...
단순한 기능 구현을 넘어, 실제 서비스 운영과 안정성을 고려하는 백엔드 설계의 중요성을 깊이 배웠습니다.

- 로그인을 처리할 때 일반 로그인과 소셜 로그인 방식과 무관하게 계정 잠금, 휴면 처리, 탈퇴 여부를
일관된 기준으로 제어해야 하는 것을 경험했습니다. 이를 통해 식별 및 인증뿐만 아니라 사용자 상태와 보안 요소를 종합적으로 고려하는
아키텍처 설계의 필요성을 배웠습니다.

- 스케줄러 과다 실행이라는 상황을 겪으면서 애플리케이션 단의 제어뿐만 아니라 데이터베이스 쿼리 레벨에서의 데이터 무결성 확보가 시스템 장애를 예방한다는 것을
경험했습니다.

- 테스트가 끝난 코드를 실서버에 배포할 때는 실제 비즈니스 흐름에 맞춰 스케줄러 실행 주기를 조정하는 등 서버 자원 낭비를 최소화하는
점검 과정이 필수적임을 깨달았습니다. 
